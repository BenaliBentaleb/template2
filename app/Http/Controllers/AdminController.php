...
class AdminController extends Controller
{
    ...
    public function modifier_utilisateur($id, Request $request)
    {
        ...
        if ($request->has($roles) == false) {
            ...
            if (!$request->has('Administrateur') && !$request->has('Enseignant') && !$request->has('Etudiant') && !$request->has('Gérant-club')) {
                $role = new Role;
                $role->user_id = $user->id;
                $role->nom = 'Etudiant';
                $role->save();
        ...
        /*  if ($request->Etudiant == null) {

        }*/

        foreach ($roles as $r) {
            if ($request->$r && ($request->$r != "Etudiant")) {
